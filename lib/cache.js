...
cp.revalidate = function (requestInfo) {
    ...
    if (self._requestInfoRevalidating.get(requestInfo.hash)) {
        if (self._debug) {
            self._log.debug("Updated requestInfo for revalidating with hash: " + requestInfoHash);
        }
    } else {
        if (self._debug) {
...
cp.validated = function (requestInfo) {
    ...
    if (self._debug) {
        self._log.debug("Evicted requestInfo from revalidate with hash: " + requestInfoHash);
    }
