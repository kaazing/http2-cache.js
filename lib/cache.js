...
function satisfiesRequest(requestInfo, candidate, cadidateHash) {
    return getResponseHash(requestInfo, candidate) === cadidateHash;
}
    ...
    return new Promise(function (resolve, reject) {
        ...
        if (!(requestInfo instanceof RequestInfo)) {
            ...
            if (
                ...
                var candidate, cadidateHash,
                    candidates = self._requestInfoToResponses.get(requestInfo.hash) || new Dict(),
                    cadidatesHashs = candidates.keys();

                while (
                    response === null &&
                        (cadidateHash = cadidatesHashs.next().value)
                ) {
                    candidate = candidates.get(cadidateHash);

                    if (
                        candidate && 
                            satisfiesRequest(requestInfo, candidate, cadidateHash) && 
