        ...
        if (!(requestInfo instanceof RequestInfo)) {
            ...
            if (
                ...
                while (
                    ...
                    if (
                        candidate && 
                            satisfiesRequest(requestInfo, candidate, candidateHash.value)
                    ) {
                        response = candidate;
                    }
                }                
...
cp.revalidate = function (requestInfo) {
    ...
    if (self._requestInfoRevalidating.get(requestInfo.hash)) {
        throw new Error("Invalid requestInfo already stale");
    }

    if (self._debug) {
        self._log.debug("Added requestInfo to revalidate with hash: " + requestInfoHash);
    }

    self._requestInfoRevalidating.set(requestInfo.hash, requestInfo);
};
...
cp.validated = function (requestInfo) {
    ...
    if (self._debug) {
        self._log.debug("Evicted requestInfo to revalidate with hash: " + requestInfoHash);
    }

    self._requestInfoRevalidating.delete(requestInfo.hash, requestInfo);
};
