...
function enableXHROverH2(xhrProto, configuration) {
    ...
    // Prepare getter with cache
    var getResponseTypeFromOptions = function (responseType, options) {
        if (options && options.response) {
            return dataToTypeCached(options.response.data, responseType);
        }
    };
    ...
    definePrivate(xhrProto, '_changeState', function (state, options) {
        ...
        // Defined only once and tolerate reset
        if (self.response === null || self.response === undefined) {
            Object.defineProperty(self, 'response', {
                enumerable: true,
                configurable: true,
                get: function () {
                    return getResponseTypeFromOptions(self.responseType, options);
                }
            });

            // TODO set self.responseType default
            // TODO only if self.responseType === 'text'
