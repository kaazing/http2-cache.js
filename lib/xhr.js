    ...
    definePrivate(xhrProto, '_changeState', function (state, options) {
        ...
        switch (state) {
            ...
            case XMLHttpRequest.DONE:
                var headers = options.response.headers, 
                    responseHeaders = {};
                keys(headers).forEach(function (header) {
                ...
                redefine(this, 'getAllResponseHeaders', function() {
                    return keys(responseHeaders).map(function (header) {
                        return header + ': ' + responseHeaders[header];
                    }).join("\n");
                });
        ...
        xhrChannel.port1.onmessage = function (event) {
            ...
            if (
                ...
                if (data.method === 'willSendViaChannel') {
                    ...
                    if (data.params.options.response) {

                        // Use buffer has data
                        if (data.params.options.response.buffer) {
                            data.params.options.response.data = data.params.options.response.buffer;
                        }   

                        // Export cached response
                        defineXhrResponse(self, data.params.options.response);
                    }
