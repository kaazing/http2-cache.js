...
function enableXHROverH2(XMLHttpRequest, configuration) {
    ...
    function defineXhrResponse(xhr, response) {
        ...
        defineGetter(xhr, 'response', function () {
            ...
            return dataToType(response.data, xhr.responseType || 'text');
        });
    ...
    definePrivate(xhrProto, '_sendViaHttp2', function (destination, body, proxyTransportUrl) {
        ...
        configuration.cache.match(requestInfo).then(function (response) {
            if (response) {
                ...
                var request = http2.raw.request({
                    ...
                    response.on('finish', function () {
                        ...
                        configuration.cache.put(requestInfo, response).then(function () {
                ...
                // TODO why configuration.js line 240 ?
    ...
    redefine(xhrProto, 'send', function (body) {
        ...
        if (configuration.isConfiguring()) {
            ...
            if (proxyTransportUrl) {
                ...
                // Reset response|responseText
                definePublic(XMLHttpRequest, 'response', self._response || self.response);
                definePublic(XMLHttpRequest, 'responseText', self._responseText || self.responseText);
