...
function enableXHROverH2(XMLHttpRequest, configuration) {
    ...
    definePrivate(xhrProto, '_sendViaHttp2', function (destination, body, proxyTransportUrl) {
        ...
        cache.match(requestInfo).then(function (cachedResponse) {
            ...
            var responseHeader = cachedResponse !== null ? cachedResponse.headers : null,
                revalidateResponse = false;//responseHeader !== null && responseHeader.etag;
            ...
            if (cachedResponse !== null && revalidateResponse === false) {
                ...
                if (cachedResponse !== null && revalidateResponse === true) {
                    ...
                    if (
                        ...
                        requestHeaders.set('if-none-match', responseHeader.etag);
                    }
                    ...
                    if (
                        ...
                        if (['GET', 'HEAD'].indexOf(requestMethod) !== -1) {
                            requestHeaders.set('if-modified-since', Date.now());
                        } else {
                            requestHeaders.set('if-unmodified-since', Date.now());   
                        }
                    }
                }
                ...
                var request = http2.raw.request({
                    ...
                    console.log('newResponse', responseHeader, newResponse.statusCode)
                    if (
                        cachedResponse !== null && 
                        ...
                        // TODO update cachedResponse with newResponse  
                        if (newResponse.headers.has('cache-control') === true) {
                            cachedResponse.headers.set('cache-control', newResponse.headers.get('cache-control'));
                        }

                        cache.put(requestInfo, cachedResponse).then(function (cachedResponse) {
