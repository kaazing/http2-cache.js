...
function enableXHROverH2(xhrProto, configuration) {
    ...
    redefine(xhrProto, 'open', function (method, url, async, username, password) {
        ...
        /*
         * We need to fire opened event here but don't native library might
    ...
    redefine(xhrProto, 'send', function (body) {
        ...
        if (configuration.isConfiguring()) {
            ...
            if (proxyTransportUrl) {
                ...
                // restore(self, 'addEventListener');
                // restore(self, 'removeEventListener');
                // restore(self, 'dispatchEvent');
    ...
    definePrivate(xhrProto, 'subscribe', function (cb) {
        ...
        var destination = parseUrl(this.__url);
        var requestInfo = new RequestInfo(this.__method, getOrigin(destination.href) + destination.path);
