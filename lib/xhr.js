...
var definePrivate = require('./utils').definePrivate;
...
var XhrInfo = new require('./xhr-info.js');
...
function enableXHROverH2(xhrProto, configuration) {
    ...
    redefine(xhrProto, 'setRequestHeader', function (name, value) {
        ...
        xhrInfo.get(this, 'headers', {})[name] = value;
    });
                ...
                configuration._cache.put(requestInfo, new Promise(function (resolve, reject) {
                    if (body) {
                        ...
                        if (body instanceof HTMLElement) {
                            if (!xhrInfo.get(self, 'headers', {})['Content-Encoding']) {
                                xhrInfo.get(self, 'headers')['Content-Encoding'] = 'UTF-8';
                            }
                            if (!xhrInfo.get(self, 'headers')['Content-Type']) {
                                xhrInfo.get(self, 'headers')['Content-Type'] = 'text/html; charset=utf-8';
                            }
                        } else {
    ...
    definePrivate(xhrProto, 'subscribe', function (cb) {
        ...
        this.__subscription = function (request) {
            // TODO should we go through xhr lifecycle again ??
            if (request.key === requestInfo.key) {
                cb();
            }
        };
        configuration.getCache().on('cached', this.__subscription);
    });

    definePrivate(xhrProto, 'unsubscribe', function () {
        configuration.getCache().removeListener('cached', this.__subscription);
    });
}
