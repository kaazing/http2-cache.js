    ...
    redefine(xhrProto, 'send', function (body) {
        ...
        if (configuration.isConfiguring()) {
            ...
            if (proxyTransportUrl) {
                ...
                // Let pending request proceed if proxyTransportUrl match
                if (options.accelerationStrategy === 'connected') {
                    configuration.on('pushOpenFailure', function pushOpenFailure(pushUrl, transportUrl) {
                        // Check transport match
                        if (transportUrl === proxyTransportUrl) {
                            // Remove listener pushOpenFailure on match only 
                            configuration.off('pushOpenFailure', pushOpenFailure);
                            // Re-attempt send request via native stack directly
                            self._send(body);   
                        }
                    });
                }
