...
function enableXHROverH2(xhrProto, configuration) {
            ...
            function (response) {
                if (response) {
                    if (configuration.debug) {
                        console.log("Using cache result for XHR(" + destination + ")");
                    }
                    ...
                    var request = http2.raw.request({
                        ...
                        response.on('finish', function () {
                            ...
                            configuration.cache.put(requestInfo, response).catch(function () {
                                // NOOP
                            });
                        });
                    });
    ...
    definePrivate(xhrProto, 'subscribe', function (cb) {
        ...
        var url = parseUrl(xhrInfo.get(this, 'url'));
        ...
        configuration.cache.on('cached', subscription);

        definePrivate(xhrProto, 'unsubscribe', function () {
            configuration.cache.removeListener('cached', subscription);
        });
    });
}
