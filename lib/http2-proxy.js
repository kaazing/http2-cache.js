...
////////////////////////////////////////////////// XMLHttpRequest ////////////////////////////////////////////////////
    ...
    redefine(xhrProto, 'open', function (method, dUrl, async, username, password) {
        ...
        // We defer state change because we may use native XHR
    ...
    redefine(xhrProto, 'send', function (body) {
        ...
        if (configuring.configuring()) {
            configuring.on('completed', function(){
                self.send(body)
            });
        } else {
            ...
            if(proxyTransportUrl){
                ...
                if(isCached){
                    ...
                    var request = http2.raw.request({
                        ...
                        self._changeState('headers_received', response);
                        response.on('data', function () {
                            self._changeState('loading');
                        });
                        response.on('finish', function () {
                            self._changeState('done');
                        });
                    });
        ...
        value: function(s, args){
            switch(s) {
                ...
                case "headers_received":
                    this._state = s;
                    redefine(this, 'status', args.statusCode);
                    redefine(this, 'readyState', 2);
                    this.onreadystatechange();
                    break;
                case "loading":
