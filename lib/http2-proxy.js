(function () {
    ...
    var ConfEmitter = require('./h2-utils').ConfEmitter;
    var define = require('./h2-utils').define;
    var getOrigin = require('./h2-utils').getOrigin;
    var resolvePort = require('./h2-utils').resolvePort;
    var parseUrl = require('./h2-utils').parseUrl;
    ...
    var proxyMap = {};  // host to proxy ws address
    var xhrProto = XMLHttpRequest.prototype;
    var cache = new Cache();
    var Promise = require("bluebird");
    var configuring = new ConfEmitter();
    ...
    function onPush(pushRequest, origin, cache) {
        cache.match(pushRequest).then(function (response) {
            console.warn("Server pushed a non cacheable response or one that is already cached");
            pushRequest.close();
        }, function () {
    ...
    function openH2StreamForPush(hostname, port, path, transport, origin) {
        ...
        var request = http2.raw.request({
            ...
            response.on('finish', function () {
                ...
                openH2StreamForPush(hostname, port, path, transport, origin);
            });
        });

        // add to cache when receive pushRequest
        request.on('push', function (pushRequest) {
            onPush(pushRequest, origin, cache);
        });
        request.end();
    }
    ...
    function addConfig(config) {
        ...
        proxyMap[origin] = proxyTransportUrl;

        if (proxyH2PushPath) {
            var wsTransport = getTransport(proxyTransportUrl);
            openH2StreamForPush(proxyUrl.hostname, resolvePort(proxyUrl), proxyH2PushPath, wsTransport, origin);
        }
    }
...
////////////////////////////////////////////////// XMLHttpRequest ////////////////////////////////////////////////////
    ...
    define(xhrProto, 'send0', function send0(destination, body, proxyTransportUrl) {
        var self = this;
        var requestInfo = new RequestInfo(self.__method, getOrigin(destination.href) + destination.path);

        // var _onCachedResponse = function (response) {
        ...
        // };
        cache.match(requestInfo).bind(self).then(
            ...
            function () {
                ...
                cache.put(requestInfo, new Promise(function (resolve, reject) {
                    ...
                    var request = http2.raw.request({
                        ...
                        transport: function () {
                            return getTransport(proxyTransportUrl);
                        }
    ...
    define(xhrProto, 'send', function (body) {
        ...
        if (configuring.configuring()) {
            ...
            if (proxyTransportUrl) {
                self.send0(destination, body, proxyTransportUrl);
            } else {
