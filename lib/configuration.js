...
confProto.openH2StreamForPush = function (pushUrl, proxyTransportUrl) {
    ...
    var transport = that.getTransport(proxyTransportUrl),
        ...
        openH2StreamForPush = function () {
            ...
            if (pushRequest.connected) {
                throw new Error("Server push stream already connected for push "+ pushUri.href);
            ...
            var request = http2.raw.request({
                ...
                // Set push request open
                pushRequest.connected = true;
                ...
                response.on('error', function (err) {
                    that._log.error("Push channel response error: ", err);
                    // Reconnect on error
                    reopenH2StreamForPush(err);
                });
            });
