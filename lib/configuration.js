            ...
            if (data.method === 'addConfig') {
                ...
                xhr.addEventListener('readystatechange', function () {
                    ...
                    if (state === XMLHttpRequest.HEADERS_RECEIVED) {
                        options.response.headers = (xhr.responseRaw ? xhr.responseRaw.headers : parseResponseHeaders(xhr.getAllResponseHeaders() || ""));
                        options.response.statusCode = (xhr.responseRaw ? xhr.responseRaw.statusCode : 200);
                    }

                    if (state >= XMLHttpRequest.LOADING) {
                        options.response.data = (xhr.responseRaw ? xhr.responseRaw.data : String(xhr.response));
                    }

                    // Use Sharebuffer to prevent copy
                    var transferable = [],
                        // TODO Enable via config ? 
                        useTransferable = false; // Pass on Opera/Chrome/Safari, Fail on Firefox with "An attempt was made to use an object that is not, or is no longer"
                    ...
                    // TODO fix Firefox InvalidStateError: An attempt was made to use an object that is not, or is no longer, usable (http://localhost:8086/dist/http2-Cache.js:942)
                    if (
                        useTransferable &&
                            options.response && 
