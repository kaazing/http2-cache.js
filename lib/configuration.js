            ...
            if (data.method === 'addConfig') {
                ...
                xhr.addEventListener('readystatechange', function () {
                    var state = xhr.readyState,
                        options = {
                            response: (xhr.responseRaw ? {
                                statusCode: xhr.responseRaw.statusCode,
                                headers: xhr.responseRaw.headers,
                                data: xhr.responseRaw.data
                            } : {
                    ...
                    var transferable = [],
                        ...
                        useTransferable = true; // Pass on Opera/Chrome/Safari, Fail on Firefox with "An attempt was made to use an object that is not, or is no longer"

                    // fix Uncaught DataCloneError: Failed to execute 'postMessage' on 'Worker': Value at index 0 does not have a transferable type.
                    ...
                    // fix Uncaught (in promise) DOMException: Failed to execute 'postMessage' on 'MessagePort': An ArrayBuffer is neutered and could not be cloned.                    
                    // - https://stackoverflow.com/questions/38169672/why-are-transfered-buffers-neutered-in-javascript/38283644 
                    /// TODO Error: Firefox InvalidStateError: An attempt was made to use an object that is not, or is no longer, usable (http://localhost:8086/dist/http2-Cache.js:942)
                    if (
                        useTransferable &&
                            options.response && 
