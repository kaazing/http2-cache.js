...
confProto.openH2StreamForPush = function (pushUrl, proxyTransportUrl) {
    ...
    var openH2StreamForPush, reopenH2StreamForPush, // expression not function,
        ...
        reconnectAutoDelay = options.reconnectAutoDelay || 100,
        reconnectAuto = options.reconnectAuto || true,
        fallbackOnConnectFailure = options.fallbackOnConnectFailure || false,
        reconnectTimer =  null,
        opened = false,
        reopening = false;
    ...
    reopenH2StreamForPush = function (err) {
        ...
        if (err) {
            ...
            // Clear paths attached to pushUrl
            if (fallbackOnConnectFailure) {
                var proxyUrls = self.getTransportUrls(proxyTransportUrl);

                // Remove urls for that transport
                proxyUrls.forEach(self.removeTransportUrl.bind(self));

                // Restore urls for that transport once on open
                transport.once('open', function () {
                    proxyUrls.forEach(self.addTransportUrl.bind(self));
                });
            }
...
confProto.addConfig = function (config) {
    ...
    if (config.hasOwnProperty('clientLogLevel')) {
        this.setDebugLevel(config.clientLogLevel);
    }
    ...
    if (pushUrl) {
        this.openH2StreamForPush(pushUrl, proxyTransportUrl);
    }
};
