...
confProto.getTransport = function (url) {
    ...
    var that = this,
        hasExistingTransport = that._activeTransportConnections.hasOwnProperty(url),
        hasActiveTransport =  hasExistingTransport && !!that._activeTransportConnections[url].writable;
...
confProto.addConfig = function (config) {
    ...
    // Option on config root
    var configPossibleOptions = ['reconnectAuto', 'reconnectAuto'];
    configPossibleOptions.forEach(function (configOption) {
    ...
    if (config.hasOwnProperty('options')) {
        keys(config.options).forEach(function (option) {
            that.options[option] = config.option[option];
        });
    }
    
    var cntI = config.proxy.length;
    for(var i = 0; i < cntI; i++) {
        that.addTransportUrl(config.proxy[i], proxyTransportUrl);
    }

    if (pushUrl) {
        that.openH2StreamForPush(parseUrl(defaultPort(pushUrl)), proxyTransportUrl);
    }
};
