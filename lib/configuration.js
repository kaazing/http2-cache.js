        ...
        if (err) {
            ...
            if (options.accelerationStrategy === 'connected') {
                ...
                transport.once('open', function () {
                    proxyUrls.forEach(that.addTransportUrl.bind(that));
                });
...
confProto.addConfig = function (config) {
    ...
    // Install transport
    if (config.hasOwnProperty('proxyTransportUrl')) {
        
        // Add transport proxyfied urls 
        config.proxy.forEach(function (url) {
          that.addTransportUrl(url, config.proxyTransportUrl);
        });

        // Connect transport to push url
        if (config.hasOwnProperty('pushUrl')) {
            that.openH2StreamForPush(config.pushUrl, config.proxyTransportUrl);
        }
    }
};
