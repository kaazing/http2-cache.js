...
confProto.onPush = function (pushRequest, scheme) {
    var self = this;
    var origin = getOrigin(scheme + "://" + pushRequest.headers.host);
...
confProto.openH2StreamForPush = function (pushUrl, proxyTransportUrl) {
    ...
    var transportURL = parseUrl(proxyTransportUrl);
    var scheme;
    switch(transportURL.protocol)
    {
        case "tcp:":
        case "ws:":
            scheme = "http";
    ...
    request.on('push', function (pushRequest) {
        self.onPush(pushRequest, scheme);
    });
    request.end();
};
...
confProto.addConfig = function (config) {
    ...
    if (pushUrl) {
        this.openH2StreamForPush(parseUrl(pushUrl), proxyTransportUrl);
    }
};
